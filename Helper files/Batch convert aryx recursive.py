...
system_path_symbol = '\\'
path = None
#path = r'E:\Research_analysis\2024.09 JET\Lab_comparison_test\Raw data'
...
def main():
    ...
    answer = input('WARNING! This will recursively iterate over ALL sub-folders. \nContinue? y/n: ')
    
    if (answer == 'y') or (answer == 'Y'):
        print(f'Starting conversion in \n{path}')
        batch_convert_aryx_recursive(path)
        print('Exporting finished.')
    else:
        print('User din\'t continue')
...
# Then iterate recursively over all folders
def batch_convert_aryx_recursive(path):
    batch_convert_aryx_folder(path)
    
    # get folders in the path
    onlydirs = [f for f in os.listdir(path) if os.path.isdir(os.path.join(path, f))]
    
    for dirname in onlydirs:
        batch_convert_aryx_recursive(path + dirname + system_path_symbol)

# Read a spectrum (.aryx) and save the data into a separate .txt file
def batch_convert_aryx_folder(path):
    ...
    for filestring in filenames:
        ...
        try:
            process_spectrum(path, filestring)
...
def process_spectrum(path, filestring):
